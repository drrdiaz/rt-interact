...
function fractionationBase(row) {
  if (row.fractionation_key === "Stereotactic") {
    return "Potential severe multi-organ toxicity with stereotactic-equivalent whole-body irradiation context";
  }
  if (row.fractionation_key === "Palliative") {
    return "Potential multi-organ toxicity with palliative whole-body irradiation context";
  }
  if (row.fractionation_key === "Conventional") {
    return "Potential multi-organ toxicity with concomitant total-body irradiation";
  }
  return "Potential multi-organ toxicity with total-body irradiation";
}
...
function buildWarning(row) {
  ...
  if (LOW_CONCERN_CLASSES.has(drugClass) || row.risk_category === "Low concern") {
    return "No clear additional acute total-body irradiation interaction signal identified";
  }
  if (agent.includes("cisplatin")) {
    return `${base}; marrow, renal, pulmonary, and mucosal toxicity may increase with concurrent cisplatin`;
  }
  if (drugClass === "CDK4/6 inhibitor" || agent.includes("abemaciclib")) {
    return `${base}; profound marrow toxicity, diarrhoea, and pneumonitis may worsen tolerance`;
  }
  if (isImmune) {
    return `${base}; immune pneumonitis, hepatitis, nephritis, or enteritis may overlap with TBI toxicity`;
  }
  if (isVegf) {
    return `${base}; bleeding and healing toxicity require caution`;
  }
  if (agent.includes("cetuximab") || drugClass.includes("EGFR")) {
    return `${base}; limited direct site data, but pulmonary and mucosal toxicity may be amplified`;
  }
  if (
    ...
    return `${base}; radiosensitisation may amplify multi-organ toxicity`;
  }
  return base;
}
